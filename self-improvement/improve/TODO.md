...
3. **Improve or implement** the detector (`backend/src/concepts/`) + fact + priority +
   template(s) (`backend/src/compose/templates.ts`) + wiring (`annotate.ts`).
...
> The Spotlight (frontend/src/ui/walkthrough.ts, focus-mode walkthrough of the best
...
> touch frontend/src/ui/walkthrough.ts (exception to the no-UI rule — captions only,
> never layout).
...
- [x] U3 · API-key LLM commentary — 2026-06-12 (backend/src/llm/providers.ts generateViaApi: direct browser call to api.anthropic.com with user's own key in localStorage; haiku; R4 verify + silent fallback)
...
- [x] I2 · Recall harness — 2026-06-11 (self-improvement/test/recall.test.ts; metrics → self-improvement/improve/metrics.json; floors ratchet up)
