...
import { buildMoveReport, type MoveReport } from '@backend/analysis/report';
import { openingBook } from '@backend/analysis/openings';
import { composeComment, BAD_KINDS, MISSED_KINDS, type Comment } from '@backend/compose/compose';
import { renderFact, } from '@backend/compose/templates';
import type { Ply } from '@backend/analysis/pgn';
import type { FactKind } from '@backend/concepts/facts';

// score.ts lives at self-improvement/eval/, so ROOT is the self-improvement
// dir; the app code (frontend/src, backend/src) is one level up at the repo root.
