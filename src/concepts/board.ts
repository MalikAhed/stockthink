 ...
 * the classic iterative SEE "swap algorithm" (chessprogramming.org).
 *
 * NOTE: the swap `see()` here ignores pins. For any EXPLANATION math (claims
 * that appear in commentary) use the pin-aware, legal-move seeSquare/seeCapture
 * in primitives.ts instead — this one is only for cheap screening.
 */
import {
  attacks,
  bishopAttacks,
