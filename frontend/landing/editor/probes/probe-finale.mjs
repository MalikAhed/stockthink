// Drive finale-stage.html: dump exact square coords + screenshot the master clock at multiple t in ONE
// Chrome session. Usage: GL=sw node editor/probes/probe-finale.mjs <t1> <t2> ...   (default a beat sweep)
...
const TIMES = TS.length ? TS : [0, 3, 5, 6.5, 8, 10, 12, 14, 16, 18];
const URL = `http://localhost:${PORT}/frontend/landing/editor/probes/finale-stage.html?t=${TIMES[0]}`;
const DBG = 9322;
...
(async () => {
  try {
    ...
    let status = '';
    for (let i = 0; i < 40; i++) { await sleep(1000); status = await evalJS(`(window.__enderReady?'ready':(window.__enderErr?('ERR '+window.__enderErr):'wait'))+' | '+(document.getElementById('st')?.textContent||'')`); if (status.startsWith('ready') || status.startsWith('ERR')) break; }
    console.log('STATUS:', status);
    const coords = await evalJS('JSON.stringify(window.__coords||{})');
    console.log('COORDS:', coords);
    for (const tt of TIMES) {
      await evalJS(`window.__setT(${tt})`);
      await sleep(700);   // let a few rAF frames render the seeked state
      const { data } = await send('Page.captureScreenshot', { format: 'png' });
      const out = `/tmp/st-finale-t${String(tt).replace('.', '_')}.png`;
