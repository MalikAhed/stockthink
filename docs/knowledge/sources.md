...
- **Lichess puzzle database** — https://database.lichess.org/#puzzles
  ...
  **Use**: per-theme recall fixtures → `scripts/puzzles/fetch-fixtures.mjs`
  → `test/fixtures/puzzles/*.csv` → `test/recall.test.ts`.
...
- `docs/knowledge/chesscom-templates.md` — verbatim chess.com phrases + triggers.
  Grow by feeding interesting positions/screens, but prefer real data above for detector work.

## Research already digested

- `docs/research/RESEARCH-DIGEST.txt`, `LLM-COMMENTARY-RESEARCH.md`,
  `LAST-RESEARCH-NOTES.md`, `chess-research-papers.md`, PDFs in `last research/`.
- `docs/research/openchess-insights-chess_review.py` — reference Python
  implementation of a review pipeline (detector ideas; our code is TypeScript).

## Specs

- `docs/specs/ANALYSIS-SYSTEM-V2.md` — the implemented V2 system.
- `docs/specs/WHY-EXPLANATION-ENGINE-SPEC.md` — explanation engine spec.
