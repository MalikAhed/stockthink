...
Stockfish (fixed nodes, MultiPV 3)
  ...
  → fact sheet JSON  (src/llm/factsheet.ts — the only contract)
  → language layer: templates | on-device reworder | Claude/ChatGPT exchange
  → post-generation fact-check (src/llm/verify.ts) — reject & keep template
```
...
- Fixed nodes + MultiPV 3 — `src/analyze.ts` (TIER_NODES)
- Win% ladder + noise-floor docs — `src/analysis/classify.ts` (ladder)
- Volatility gate — `src/engine/engine.ts` (shallowEval), `src/analysis/classify.ts` (isVolatile)
- Win%-space criticality for Great — `src/analysis/classify.ts` (isGreat)
- Typed facts / refutation walk — `src/explain/` (spec §4–§8)
- Fact sheet contract — `src/llm/factsheet.ts`
- Constrained reworders — `src/llm/webllm.ts` (on-device), `src/llm/exchange.ts` (Claude/ChatGPT paste)
- Post-generation verification — `src/llm/verify.ts`
